...
def create_wrd_mp(
    ...
    @m.Constraint(
        m.fs.non_working_hours_morning,
        doc="Train 4 on/off state constant during non-working hours",
    )
    def eq_train_4_on_nwh_morning(b, h):
        if h == 0:  # or (h % 24)==0: --> this causes issues but idk why
            return Constraint.Skip
        return (
            b.fs.mp.blocks[h].process.fs.train_4_on
            == b.fs.mp.blocks[h - 1].process.fs.train_4_on
        )
...
def plot_function(m, n_time_points, season):
    ...
    ax.set_title(
        season + ": Modular Shutdown + Variable Flowrate Scenario",
        fontsize=14,
        fontweight="bold",
    )
...
if __name__ == "__main__":
    ...
    total_water_production_target = (
        0.65 * 53150 * pyunits.m**3 / pyunits.day * n_days * pyunits.day
    )
